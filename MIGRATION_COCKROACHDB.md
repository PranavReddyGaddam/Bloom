...
new `db.py` functions, first written against `supabase-py` and then rewritten against SQL. Migrating
...
- No `CREATE EXTENSION vector` needed. `feature.vector_index.enabled` was **already `true`** on
  ...
  permitted but a no-op, so B1's "enable the setting" step is not needed.
...
- Index is **k-means partitioned (C-SPANN), not HNSW**: a different implementation with the same
  interface. It is **approximate**, unlike the exhaustive scan we have today.
- `language sql` and PL/pgSQL UDFs with `RETURNS TABLE` are supported, so our match functions port
  as functions.
...
- **`dict_row` preserves the return contract.** `row_factory=psycopg.rows.dict_row` makes every
  query return `list[dict]` with string keys, which is exactly what `.execute().data` returned. This is
...
- **`pgvector.psycopg.register_vector()` covers only the read direction on CockroachDB.**
  ~~handles `list[float]` ↔ `VECTOR` both ways~~ is **wrong, verified 2026-07-25.** It succeeds
...
| `db.py:943` | embedded `documents(filename)` + `count="exact"` + `.not_.is_(…, "null")` | `JOIN documents` (inner join enforces the NOT NULL intent and matches the docstring: concepts whose source doc was deleted are skipped) + `d.filename AS document_filename`. |
| `db.py:309`, `db.py:388` | `.in_("quiz_attempt_id", attempt_ids)` | `= ANY(%s)` with the list as **one** array parameter, which avoids an unbounded IN-list, a genuine Cockroach anti-pattern. |
...
**Timestamps AND uuids: confirmed biting, and worse than first described.** psycopg returns
...
- Wrap `_store_document` in one transaction under the retry decorator. Today the
  `DELETE` (`:158`) and insert (`:166`) are separate calls, so a failure between them leaves a
...
Do **not** port the 14 migration files one-to-one. `match_concept_mastery` is defined **twice**, `migrate_concept_mastery.sql:32` creates it, `migrate_calibration.sql:15` drops and recreates it
...
**Keep all three UDFs as functions.** They are `language sql stable` single-statement SELECTs, the easiest possible port, and keeping them makes each call site a one-line change. Define
...
a 30-day throttled grace period on Aug 24, and throttled is not "without any restriction" during
